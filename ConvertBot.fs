...
let download (client: HttpClient) uri =
    async {
        if Reddit.isRedditUri uri then
            ...
            log
            <| sprintf "Downloading [%s] to [%s]..." (uri.ToString()) fileName
...
let reply config chatID fileState =
    async {
        log
        <| sprintf "Uploading %s to %i" fileState.Converted chatID
...
let convertReplyCleanup config chatId =
    let sendMessage s a =
        sprintf s a |> (sendMessageAsync config chatId)
...
let convertWebm config client (chatMessage: ChatMessage) : Async<Result<unit, string>> =
    ...
    match doc with
    | None -> Error "No file provided" |> Async.result
    | Some doc ->
        doc
        |> AsyncResult.result
...
let onUpdate config client settings (chatMessage: ChatMessage) =
    ...
    async {
        ...
        log
        <| sprintf "Message from %s: %A" chatMessage.User chatMessage.Message

        match chatMessage with
        | Command "/whitelist add" msg when isAdmin -> return! addToWhitelist config settings msg
        | Command "/whitelist remove" msg when isAdmin -> return! removeFromWhitelist config settings msg
